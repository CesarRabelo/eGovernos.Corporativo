...
namespace Platinium.Comum
{
    public class ExibirArquivo
    {
        ...
        public string NomeArquivo(string id, DateTime data)
        {
            oArquivo = new Arquivo(Convert.ToInt32(id), oDao);
            string sNomeArquivo = oArquivo.Prefixo.Trim() + string.Format("{0:" + oArquivo.Mascara + "}", data).Trim() + "." + oArquivo.GrupoArquivo.Codigo.Trim();
            return sNomeArquivo;
        }

        public DataTable DadosArquivo(string id)
        {
            oArquivo = new Arquivo(Convert.ToInt32(id), oDao);
            DataTable dtb = this.oDao.Select(new List<Parameter>(), "platinium", "vi_" + oArquivo.Codigo.Trim(), new Dictionary<string, string>());
            return dtb;
        }

        public DataTable DadosColunas(string id)
        {
            Dictionary<string, string> dicionario = Pro.Utils.ClassFunctions.GetMap(typeof(ArquivoColuna));
            List<Parameter> lstParametros = new List<Parameter>();
            lstParametros.Add(new Parameter("Arquivo", Convert.ToInt32(id), OperationTypes.EqualsTo));
            lstParametros.Add(new Parameter("Sequencia", null, OperationTypes.Null, "ASC"));
            return oDao.Select(lstParametros, "platinium", "VI_ARQUIVO_COLUNA", dicionario);
        }
    }
}
