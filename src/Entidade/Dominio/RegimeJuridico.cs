...
{
    [ClassAttribute(Schema = "platinium", Table = "TB_REGIME_JURIDICO_REJU")]
    public class RegimeJuridico
    {
        ...
        [PropertyAttribute(Field = "ID_REGIME_JURIDICO", AllowNull = false, PropertyType = PropertyCategories.PrimaryKey, Variable = "iID")]
        public int ID
        {
            get { return iID; }
        }

        [PropertyAttribute(Field = "COD_REGIME_JURIDICO", AllowNull = false, Description = "Código", PropertyType = PropertyCategories.Field)]
        public string Codigo
        {
            get;
            set;
        }
        [PropertyAttribute(Field = "DSC_REGIME_JURIDICO", AllowNull = false, Description = "Descrição", PropertyType = PropertyCategories.Field)]
        ...
        public RegimeJuridico(Dao dao)
        {
            oDao = dao;
        }

        public RegimeJuridico(int id, Dao dao)
        {
        ...
        private void ValidarCodigoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_REGIME_JURIDICO_REJU", typeof(RegimeJuridico)).Rows.Count != 0)
                throw new RegraNegocioException("Regime Jurídico já cadastrado com o código informado");
        }
        ...
        private void ValidarDescricaoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_REGIME_JURIDICO_REJU", typeof(RegimeJuridico)).Rows.Count != 0)
                throw new RegraNegocioException("Regime Jurídico já cadastrado com a descrição informada");
        }
