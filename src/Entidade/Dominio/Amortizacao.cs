...
{
    [ClassAttribute(Schema = "platinium", Table = "TB_AMORTIZACAO_AMOR")]
    public class Amortizacao
    {
        ...
        [PropertyAttribute(Field = "ID_AMORTIZACAO", AllowNull = false, PropertyType = PropertyCategories.PrimaryKey, Variable = "iID")]
        public int ID
        {
            get { return iID; }
        }

        [PropertyAttribute(Field = "COD_AMORTIZACAO", AllowNull = false, Description = "Código", PropertyType = PropertyCategories.Field)]
        public string Codigo
        {
            get;
            set;
        }
        [PropertyAttribute(Field = "DSC_AMORTIZACAO", AllowNull = false, Description = "Descrição", PropertyType = PropertyCategories.Field)]
        ...
        #region Construtores
        public Amortizacao() { }
        public Amortizacao(Dao dao)
        {
            oDao = dao;
        }

        public Amortizacao(int id, Dao dao)
        {
        ...
        private void ValidarCodigoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_AMORTIZACAO_AMOR", typeof(Amortizacao)).Rows.Count != 0)
                throw new RegraNegocioException("Amortização já cadastrado com o código informado");
        }
        ...
        private void ValidarDescricaoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_AMORTIZACAO_AMOR", typeof(Amortizacao)).Rows.Count != 0)
                throw new RegraNegocioException("Amortização já cadastrado com a descrição informada");
        }
