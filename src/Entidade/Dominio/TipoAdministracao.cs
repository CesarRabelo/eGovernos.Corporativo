...
{
    [ClassAttribute(Schema = "platinium", Table = "TB_TIPO_ADMINISTRACAO_TIAD")]
    public class TipoAdministracao
    {
        ...
        [PropertyAttribute(Field = "ID_TIPO_ADMINISTRACAO", AllowNull = false, PropertyType = PropertyCategories.PrimaryKey, Variable = "iID")]
        public int ID
        {
            get { return iID; }
        }

        [PropertyAttribute(Field = "COD_TIPO_ADMINISTRACAO", AllowNull = false,Description="Código", PropertyType = PropertyCategories.Field, Size = 3)]
        public string Codigo
        {
            get;
            set;
        }
        [PropertyAttribute(Field = "DSC_TIPO_ADMINISTRACAO", AllowNull = false,Description="Descrição", PropertyType = PropertyCategories.Field, Size = 50)]
		...
		public TipoAdministracao(Dao dao)
		{
			oDao = dao;
		}

        public TipoAdministracao(int id, Dao dao)
		{
        ...
        private void ValidarCodigoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_TIPO_ADMINISTRACAO_TIAD", typeof(TipoAdministracao)).Rows.Count != 0)
                throw new RegraNegocioException("Tipo administração já cadastrado com o código informado");
        }
        ...
        private void ValidarDescricaoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_TIPO_ADMINISTRACAO_TIAD", typeof(TipoAdministracao)).Rows.Count != 0)
                throw new RegraNegocioException("Tipo administração já cadastrado com a descrição informada");
        }
