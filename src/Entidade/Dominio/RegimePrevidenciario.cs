...
{
    [ClassAttribute(Schema = "platinium", Table = "TB_REGIME_PREVIDENCIARIO_REPR")]
    public class RegimePrevidenciario
    {
        ...
        [PropertyAttribute(Field = "ID_REGIME_PREVIDENCIARIO", AllowNull = false, PropertyType = PropertyCategories.PrimaryKey, Variable = "iID")]
        public int ID
        {
            get { return iID; }
        }

        [PropertyAttribute(Field = "COD_REGIME_PREVIDENCIARIO", AllowNull = false, Description = "Código", PropertyType = PropertyCategories.Field)]
        public string Codigo
        {
            get;
            set;
        }
        [PropertyAttribute(Field = "DSC_REGIME_PREVIDENCIARIO", AllowNull = false, Description = "Descrição", PropertyType = PropertyCategories.Field)]
        ...
        #region Construtores
        public RegimePrevidenciario() { }
        public RegimePrevidenciario(Dao dao)
        {
            oDao = dao;
        }

        public RegimePrevidenciario(int id, Dao dao)
        {
        ...
        private void ValidarCodigoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_REGIME_PREVIDENCIARIO_REPR", typeof(RegimePrevidenciario)).Rows.Count != 0)
                throw new RegraNegocioException("Regime Previdenciário já cadastrado com o código informado");
        }
        ...
        private void ValidarDescricaoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_REGIME_PREVIDENCIARIO_REPR", typeof(RegimePrevidenciario)).Rows.Count != 0)
                throw new RegraNegocioException("Regime Previdenciário já cadastrado com a descrição informada");
        }
