...
{
    [ClassAttribute(Schema = "platinium", Table = "TB_SUBFUNCAO_SUBF")]
    public class SubFuncao
    {
        ...
        [PropertyAttribute(Field = "ID_SUBFUNCAO_SUBF", AllowNull = false, PropertyType = PropertyCategories.PrimaryKey, Variable = "iID")]
        public int ID
        {
            get { return iID; }
        }
        ...
        [PropertyAttribute(Field = "COD_SUBFUNCAO", AllowNull = false, Description="Código", PropertyType = PropertyCategories.Field)]
        public string Codigo
        {
            get;
            set;
        }
        [PropertyAttribute(Field = "DSC_SUBFUNCAO", AllowNull = false, Description="Descrição", PropertyType = PropertyCategories.Field)]
        ...
        #region Construtores
        public SubFuncao() { }
		public SubFuncao(Dao dao)
		{
			oDao = dao;
		}

        public SubFuncao(int id, Dao dao)
		{
        ...
        private void ValidarCodigoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_SUBFUNCAO_SUBF", typeof(SubFuncao)).Rows.Count != 0)
                throw new RegraNegocioException("Subfunção já cadastrada com o código informado");
        }
        ...
        private void ValidarDescricaoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_SUBFUNCAO_SUBF", typeof(SubFuncao)).Rows.Count != 0)
                throw new RegraNegocioException("Subfunção já cadastrada com a descrição informada");
        }
