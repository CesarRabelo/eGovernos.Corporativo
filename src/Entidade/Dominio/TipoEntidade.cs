...
{
    [ClassAttribute(Schema = "platinium", Table = "TB_TIPO_ENTIDADE_TIEN")]
    public class TipoEntidade
    {
        ...
        [PropertyAttribute(Field = "ID_TIPO_ENTIDADE_TIEN", AllowNull = false, PropertyType = PropertyCategories.PrimaryKey, Variable = "iID")]
        public int ID
        {
            get { return iID; }
        }

        [PropertyAttribute(Field = "COD_TIPO_ENTIDADE", Description="Código", AllowNull = false, PropertyType = PropertyCategories.Field)]
        public string Codigo
        {
            get;
            set;
        }
        [PropertyAttribute(Field = "DSC_TIPO_ENTIDADE", Description="Descrição", AllowNull = false, PropertyType = PropertyCategories.Field)]
        ...
        #region Construtores
        public TipoEntidade() { }
		public TipoEntidade(Dao dao)
		{
			oDao = dao;
		}

        public TipoEntidade(int id, Dao dao)
		{
        ...
        private void ValidarCodigoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_TIPO_ENTIDADE_TIEN", typeof(TipoEntidade)).Rows.Count != 0)
                throw new RegraNegocioException("Campo já cadastrada com o código informado");
        }
        ...
        private void ValidarDescricaoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_TIPO_ENTIDADE_TIEN", typeof(TipoEntidade)).Rows.Count != 0)
                throw new RegraNegocioException("Campo já cadastrada com a descrição informada");
        }
