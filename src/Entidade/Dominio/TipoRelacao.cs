...
{
    [ClassAttribute(Schema = "platinium", Table = "TB_TIPO_RELACAO_TIRE")]
    public class TipoRelacao
    {
        ...
        [PropertyAttribute(Field = "ID_TIPO_RELACAO", AllowNull = false, PropertyType = PropertyCategories.PrimaryKey, Variable = "iID")]
        public int ID
        {
            get { return iID; }
        }

        [PropertyAttribute(Field = "COD_TIPO_RELACAO", AllowNull = false, Description = "Código", PropertyType = PropertyCategories.Field)]
        public string Codigo
        {
            get;
            set;
        }
        [PropertyAttribute(Field = "DSC_TIPO_RELACAO", AllowNull = false, Description = "Descrição", PropertyType = PropertyCategories.Field)]
        ...
        #region Construtores
        public TipoRelacao() { }
        public TipoRelacao(Dao dao)
        {
            oDao = dao;
        }

        public TipoRelacao(int id, Dao dao)
        {
        ...
        private void ValidarCodigoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_TIPO_RELACAO_TIRE", typeof(TipoRelacao)).Rows.Count != 0)
                throw new RegraNegocioException("Tipo de relação já cadastrada com o código informado");
        }
        ...
        private void ValidarDescricaoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_TIPO_RELACAO_TIRE", typeof(TipoRelacao)).Rows.Count != 0)
                throw new RegraNegocioException("Tipo de relação já cadastrada com a descrição informada");
        }
