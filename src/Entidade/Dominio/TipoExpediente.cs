...
{
    [ClassAttribute(Schema = "platinium", Table = "TB_TIPO_EXPEDIENTE_TIEX")]
    public class TipoExpediente
    {
        ...
        [PropertyAttribute(Field = "ID_TIPO_EXPEDIENTE", AllowNull = false, PropertyType = PropertyCategories.PrimaryKey, Variable = "iID")]
        public int ID
        {
            get { return iID; }
        }

        [PropertyAttribute(Field = "COD_TIPO_EXPEDIENTE", AllowNull = false, Description = "Código", PropertyType = PropertyCategories.Field)]
        public string Codigo
        {
            get;
            set;
        }
        [PropertyAttribute(Field = "DSC_TIPO_EXPEDIENTE", AllowNull = false, Description = "Descrição", PropertyType = PropertyCategories.Field)]
        ...
        public TipoExpediente(Dao dao)
        {
            oDao = dao;
        }

        public TipoExpediente(int id, Dao dao)
        {
        ...
        private void ValidarCodigoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_TIPO_EXPEDIENTE_TIEX", typeof(TipoExpediente)).Rows.Count != 0)
                throw new RegraNegocioException("Tipo de expediente já cadastrado com o código informado");
        }
        ...
        private void ValidarDescricaoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_TIPO_EXPEDIENTE_TIEX", typeof(TipoExpediente)).Rows.Count != 0)
                throw new RegraNegocioException("Tipo de expediente já cadastrado com a descrição informada");
        }
