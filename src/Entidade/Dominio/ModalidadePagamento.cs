...
{
    [ClassAttribute(Schema = "platinium", Table = "TB_MODALIDADE_PAGAMENTO_MOPA")]
    public class ModalidadePagamento
    {
        ...
        [PropertyAttribute(Field = "ID_MODALIDADE_PAGAMENTO", AllowNull = false, PropertyType = PropertyCategories.PrimaryKey, Variable = "iID")]
        public int ID
        {
            get { return iID; }
        }

        [PropertyAttribute(Field = "COD_MODALIDADE_PAGAMENTO", AllowNull = true, Description = "Código", PropertyType = PropertyCategories.Field, Size=2)]
        public string Codigo
        {
            get;
            set;
        }
        [PropertyAttribute(Field = "DSC_MODALIDADE_PAGAMENTO", AllowNull = false, Description = "Descrição", PropertyType = PropertyCategories.Field, Size=50)]
        ...
        public ModalidadePagamento(Dao dao)
        {
            oDao = dao;
        }

        public ModalidadePagamento(int id, Dao dao)
        {
        ...
        private void ValidarCodigoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_MODALIDADE_PAGAMENTO_MOPA", typeof(ModalidadePagamento)).Rows.Count != 0)
                throw new RegraNegocioException("Modalidade de Pagamento já cadastrado com o código informado");
        }
        ...
        private void ValidarDescricaoCadastrado()
        {
            ...
            if (oDao.Select(parametro, "platinium", "TB_MODALIDADE_PAGAMENTO_MOPA", typeof(ModalidadePagamento)).Rows.Count != 0)
                throw new RegraNegocioException("Modalidade de Pagamento já cadastrado com a descrição informada");
        }
